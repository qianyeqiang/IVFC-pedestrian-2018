...
def process_image(img, select_threshold=0.2, nms_threshold=.05, net_shape=(512, 512)):
    # Run SSD network.
...
def image_process(cv2_img):
    ...
    height = imshape[0]
    width = imshape[1]
    bbox_pedestrian = Float64MultiArray()
    ...
    if len(rbboxes) != 0:
        ...
        #xmax = int(bbox[3] * width)
                ...
                bbox_pedestrian.data =rbboxes.reshape(-1).tolist()
                #print(bbox_pedestrian.data)
        #bbox_pedestrian.data = [xmin, ymin, xmax, ymax]
        #bbox_pedestrian.data.append(xmin, ymin, xmax, ymax)
        
        #print(bbox)
        #bbox_pedestrian.data.append(12)
        #bbox_pedestrian.data.append(122)   
    else:
        bbox_pedestrian.data = [0]
        
  

    pedestrian_pub.publish(bbox_pedestrian)
...
def main():
    ...
    # publish results
    pedestrian_pub = rospy.Publisher("/pedestrian", Float64MultiArray, queue_size=1)
    # Spin until ctrl + c
    rospy.spin()
